-- builds.lua  (v2: share mode usa jugador seleccionado manualmente)
local BuildsModule = {}
...
function BuildsModule.init(ENV)
    ...
    -- ─── SHARE MODE ──────────────────────────────────────────────────────────────
    -- Cuando está activo, usa el dataFolder y userFolder del jugador seleccionado
    -- en el picker (curUN / curUID), que el usuario elige manualmente.
    -- NO intenta auto-detectar al líder.
    local shareModeActive = false

    -- Obtiene el dataFolder de cualquier jugador replicando la misma ruta que dataFolder del LP.
    -- dataFolder es algo como Players[LP.Name].PlayerData o Players[LP.Name]:FindFirstChild("Data")
    local function getDataFolderFor(playerName)
        if playerName == LP.Name then return dataFolder end
        local ok, result = pcall(function()
            local pObj = Players:FindFirstChild(playerName)
            if not pObj then return nil end
            -- Replicar la misma jerarquía que dataFolder tiene bajo LP
            if dataFolder and dataFolder.Parent then
                local parentName = dataFolder.Parent.Name  -- e.g. "PlayerData"
                local folderName = dataFolder.Name         -- e.g. "Data" / "Blocks"
                -- Caso 1: Players[name][parentName][folderName]
                if dataFolder.Parent.Parent and dataFolder.Parent.Parent == Players:FindFirstChild(LP.Name) then
                    local pa = pObj:FindFirstChild(parentName)
                    if pa then
                        local da = pa:FindFirstChild(folderName)
                        if da then return da end
                    end
                end
                -- Caso 2: Players[name][folderName]  (un nivel directo)
                local direct = pObj:FindFirstChild(folderName)
                if direct then return direct end
                -- Caso 3: mismo parent name directo
                local pa2 = pObj:FindFirstChild(parentName)
                if pa2 then return pa2 end
            end
            return nil
        end)
    ...
    local function sec(parent, order)
        local f = mk("Frame", parent, {
            Size=UDim2.new(1,0,0,1), AutomaticSize=Enum.AutomaticSize.Y,
            BackgroundColor3=T.card, BorderSizePixel=0, LayoutOrder=order or 0
        })
        corner(f,8); pad(f,10,10,8,10)
        mk("UIListLayout", f, {Padding=UDim.new(0,6), SortOrder=Enum.SortOrder.LayoutOrder})
        return f
    end
    ...
    local function buildCenter(sv)
        ...
        for _,pd in ipairs(sv.data.Block) do
            local p = Vector3.new(pd.RelX or 0, pd.RelY or 0, pd.RelZ or 0)
    ...
    local function curPlacePos()
        ...
        if z then return z.Position + Vector3.new(0, z.Size.Y/2+1, 0) end
        return myRefPos()
    end

    -- ─── HANDLES ─────────────────────────────────────────────────────────────────
    local saveDummy = mk("Part", envF, {
        Size=Vector3.new(4,4,4), Transparency=1, Anchored=true,
        CanCollide=false, CanQuery=false, Material=Enum.Material.Plastic,
        Position=Vector3.new(0,-9999,0)
    })
    local SaveHandles = mk("Handles", SG, {
        Adornee=saveDummy, Style=Enum.HandlesStyle.Movement,
        Color3=PURPLE_GAMER, Visible=false,
    })
    pcall(function() SaveHandles.AlwaysOnTop = true end)

    local saveArcAdornee = mk("Part", envF, {
        Size=Vector3.new(12,12,12), Transparency=1, Anchored=true,
        CanCollide=false, CanQuery=false, Material=Enum.Material.Plastic,
        Position=Vector3.new(0,-9999,0)
    })
    local SaveArc = mk("ArcHandles", SG, {
        Adornee=saveArcAdornee, Color3=PURPLE_GAMER, Visible=false,
    })
    ...
    renderSavePrev = function()
        ...
        for _,pd in ipairs(blocks) do
            ...
            if not p then
                p = mk("Part", savePrevF, {
                    ...
                    CanTouch=false, CastShadow=false, Material=Enum.Material.Plastic
                })
                savePool[n] = p
            end
            local relPos = Vector3.new(pd.RelX, pd.RelY, pd.RelZ)
            local relRot = CFrame.Angles(math.rad(pd.RotX or 0), math.rad(pd.RotY or 0), math.rad(pd.RotZ or 0))
            local rPA    = delta * relPos
            local rRA    = (delta - delta.Position) * relRot
            local offset = (rPA - cAdj) * placeScale
            local rotOff = placeRot * offset
            local bCF    = CFrame.new(pos + rotOff) * placeRot * rRA
            local bSz    = Vector3.new(pd.SizeX or 2, pd.SizeY or 2, pd.SizeZ or 2) * placeScale
        ...
        if minv.X ~= math.huge then
            local sz = maxv - minv
            saveDummy.Size      = Vector3.new(math.clamp(sz.X,4,200), math.clamp(sz.Y,4,200), math.clamp(sz.Z,4,200))
            saveArcAdornee.Size = saveDummy.Size
        else
            saveDummy.Size = Vector3.new(4,4,4)
        end
        saveDummy.CFrame      = CFrame.new(pos) * placeRot
        saveArcAdornee.CFrame = CFrame.new(pos) * placeRot
        updSaveHandles()
    ...
    do
        ...
        SaveHandles.MouseButton1Down:Connect(function()
            if not PageSave.Visible or not selSaveIdx then return end
            sDrag=true; sDragOP=curPlacePos(); sSavedCam=Camera.CFrame
            Camera.CameraType=Enum.CameraType.Scriptable
        end)
        SaveHandles.MouseDrag:Connect(function(face, dist)
            if not sDrag or not sDragOP then return end
            local st = tonumber(SaveUI.saveMoveStep and SaveUI.saveMoveStep.Text) or 0
            if st < 0 then st = 0 end
            local d   = (st>0) and (math.floor(dist/st+0.5)*st) or dist
            local dir = (CFrame.new(sDragOP)*placeRot):VectorToWorldSpace(Vector3.FromNormalId(face))
            placePosV = sDragOP + dir*d; useCZ = false; renderSavePrev()
        end)
        SaveHandles.MouseButton1Up:Connect(function()
            if not sDrag then return end; sDrag=false; sSavedCam=nil
            Camera.CameraType=Enum.CameraType.Custom
        end)
        SaveArc.MouseButton1Down:Connect(function()
            if not PageSave.Visible or not selSaveIdx then return end
            sArcDrag=true; sArcStartRot=placeRot; sArcSavedCam=Camera.CFrame
            Camera.CameraType=Enum.CameraType.Scriptable
        end)
        SaveArc.MouseDrag:Connect(function(axis, relAngle)
            if not sArcDrag then return end
            local av = (axis==Enum.Axis.X and Vector3.xAxis) or (axis==Enum.Axis.Y and Vector3.yAxis) or Vector3.zAxis
            local st = tonumber(SaveUI.saveRotStep and SaveUI.saveRotStep.Text) or 0
            if st < 0 then st = 0 end
            local snapped = (st>0) and math.rad(math.floor(math.deg(relAngle)/st+0.5)*st) or relAngle
            placeRot = sArcStartRot * CFrame.fromAxisAngle(av, snapped); renderSavePrev()
        end)
        SaveArc.MouseButton1Up:Connect(function()
            if not sArcDrag then return end; sArcDrag=false; sArcSavedCam=nil
            Camera.CameraType=Enum.CameraType.Custom
        end)
        RunService.RenderStepped:Connect(function()
            if sDrag and sSavedCam then Camera.CFrame=sSavedCam end
            if sArcDrag and sArcSavedCam then Camera.CFrame=sArcSavedCam end
        end)
    end
    ...
    local refreshWho
    -- setWhoBtn declarada antes que startSelTicker porque el ticker la llama
    local setWhoBtn
    do
        ...
        local function listTC(un)
            ...
            for _,p in ipairs(Players:GetPlayers()) do
                if p.Name==un and p.Team and p.Team.TeamColor then
                    local tc = p.Team.TeamColor.Color
                    return Color3.new(tc.R*0.25, tc.G*0.25, tc.B*0.25)
                end
            end
            return T.btnAlt
        end
        local function whoMembers()
            ...
            table.sort(allP, function(a,b)
                local al,bl = isLdr(a.Name), isLdr(b.Name)
            ...
            for _,p in ipairs(allP) do
                if p.Name ~= selPlayer then
                    m[#m+1] = {uid=p.UserId, dname=p.DisplayName or p.Name, uname=p.Name}
                end
            end
            return m
        end
        local function buildWhoRow(mem)
            local row = mk("Frame", whoInner, {
                Size=UDim2.new(1,0,0,36), BackgroundColor3=listTC(mem.uname), BorderSizePixel=0
            })
            corner(row, 8)
            local isL = isLdr(mem.uname)
            if isL then
                mk("ImageLabel", row, {Name="LdrIco", Size=UDim2.new(0,16,0,16),
                    Position=UDim2.new(0,3,0.5,-8), BackgroundTransparency=1, Image=ICON_LEADER, ZIndex=3})
            end
            local avX2 = isL and 22 or 5
            local av   = mk("ImageLabel", row, {Name="Avatar", Size=UDim2.new(0,26,0,26),
                Position=UDim2.new(0,avX2,0.5,-13), BackgroundColor3=T.card, BorderSizePixel=0})
            corner(av, 6)
            ...
            local nX2 = avX2 + 30
            mk("TextLabel", row, {
                Position=UDim2.new(0,nX2,0,4), Size=UDim2.new(1,-100,0,14),
                Text=mem.dname, TextColor3=T.text, BackgroundTransparency=1,
                Font=Enum.Font.GothamBold, TextSize=11, TextXAlignment=Enum.TextXAlignment.Left
            })
            mk("TextLabel", row, {
                Position=UDim2.new(0,nX2,0,20), Size=UDim2.new(1,-100,0,12),
                Text="@"..mem.uname, TextColor3=T.sub, BackgroundTransparency=1,
                Font=Enum.Font.Gotham, TextSize=9, TextXAlignment=Enum.TextXAlignment.Left
            })
            local bkL = mk("TextLabel", row, {
                Position=UDim2.new(1,-62,0,0), Size=UDim2.new(0,60,1,0),
                Text="...", TextColor3=T.sub, BackgroundTransparency=1,
            ...
            -- FIX: capturar todos los valores en variables locales propias
            ...
            rb.MouseButton1Click:Connect(function()
                ...
                whoVis = false
                if whoFrame then whoFrame.Visible = false end
                if whoBtn then
                    for _,ch in ipairs(whoBtn:GetChildren()) do
                        if ch:IsA("TextLabel") and (ch.Text=="▲" or ch.Text=="▼") then ch.Text="▼" end
                    end
                end
                -- FIX: actualizar avatar con el UID correcto del jugador seleccionado
        ...
        refreshWho = function()
            ...
            local members = whoMembers()
            local present = {}
            for _,m in ipairs(members) do present[m.uid] = m end
            for uid, rec in pairs(whoRows) do
                if not present[uid] then rec.row:Destroy(); whoRows[uid] = nil end
            end
            local order = 0
            for _,m in ipairs(members) do
                order = order + 1
                local rec = whoRows[m.uid]
                if not rec then rec = buildWhoRow(m); whoRows[m.uid] = rec end
                rec.row.LayoutOrder = order
                local tc = listTC(m.uname)
                if rec.lastTC ~= tc then rec.lastTC = tc; rec.row.BackgroundColor3 = tc end
                local bc = countPB(m.uname)
                if rec.lastCount ~= bc then
                    rec.lastCount = bc
                    rec.blockLbl.Text = bc > 0 and tostring(bc) or "0"
                    rec.blockLbl.TextColor3 = bc > 0 and T.ok or T.sub
                end
                local lIco = rec.row:FindFirstChild("LdrIco"); local isL = isLdr(m.uname)
                if isL and not lIco then
                    mk("ImageLabel", rec.row, {Name="LdrIco", Size=UDim2.new(0,16,0,16),
                        Position=UDim2.new(0,3,0.5,-8), BackgroundTransparency=1, Image=ICON_LEADER, ZIndex=3})
                elseif not isL and lIco then lIco:Destroy() end
                local av = rec.row:FindFirstChild("Avatar")
                if av then av.Position = UDim2.new(0, isL and 22 or 5, 0.5, -13) end
            end
            if #members == 0 then
                if not whoEmptyL then
                    whoEmptyL = mk("TextLabel", whoInner, {
                        Size=UDim2.new(1,0,0,24), LayoutOrder=1,
                        Text="Eres el único jugador", TextColor3=T.sub,
    ...
    -- ─── setWhoBtn: actualiza el botón principal del picker ──────────────────────
    -- FIX: usa el uid recibido como argumento, no curUID (que puede estar desactualizado)
    setWhoBtn = function(uid, dn, un)
        ...
        local isL = isLdr(un)
        if isL then
            mk("ImageLabel", whoBtn, {Name="LdrIco", Size=UDim2.new(0,14,0,14),
                Position=UDim2.new(0,4,0.5,-7), BackgroundTransparency=1, Image=ICON_LEADER, ZIndex=3})
        end
        local avX = isL and 20 or 4
        local av  = mk("ImageLabel", whoBtn, {Name="Avatar", Size=UDim2.new(0,22,0,22),
            Position=UDim2.new(0,avX,0.5,-11), BackgroundColor3=T.card, BorderSizePixel=0})
        corner(av, 6)
        local capturedUID = uid  -- capturar localmente para el spawn
        task.spawn(function()
            local ok2, url = pcall(function()
    ...
    mk("TextLabel", secSave, {
        Size=UDim2.new(1,0,0,14), Text="Seleccionar jugador",
        TextColor3=T.sub, BackgroundTransparency=1,
        Font=Enum.Font.GothamSemibold, TextSize=10,
    ...
    whoBtn = mk("TextButton", playerRow, {
        Size=UDim2.new(1,0,0,28), Position=UDim2.new(0,0,0,0),
        BackgroundColor3=T.input, BorderSizePixel=0, Text="", TextXAlignment=Enum.TextXAlignment.Left
    })
    corner(whoBtn, 8); stroke(whoBtn, T.sub, 1)
    setWhoBtn(LP.UserId, "Yo", LP.Name)

    whoFrame = mk("Frame", secSave, {
        Size=UDim2.new(1,0,0,1), AutomaticSize=Enum.AutomaticSize.Y,
        BackgroundColor3=T.card, BorderSizePixel=0, Visible=false, LayoutOrder=2
    })
    corner(whoFrame, 8); stroke(whoFrame, T.sub, 1); pad(whoFrame, 4,4,4,4)
    whoInner = mk("Frame", whoFrame, {Size=UDim2.new(1,0,0,1), AutomaticSize=Enum.AutomaticSize.Y, BackgroundTransparency=1})
    mk("UIListLayout", whoInner, {Padding=UDim.new(0,3)})
    ...
    whoBtn.MouseButton1Click:Connect(function()
        ...
        if whoVis then
            refreshWho()
            task.spawn(function()
                while whoVis and whoFrame.Parent do task.wait(1); if not whoVis then break end; refreshWho() end
            end)
        end
        for _,ch in ipairs(whoBtn:GetChildren()) do
            if ch:IsA("TextLabel") and (ch.Text=="▲" or ch.Text=="▼") then
                ch.Text = whoVis and "▲" or "▼"
            end
        end
    end)

    -- ─── SHARE MODE TOGGLE ───────────────────────────────────────────────────────
    -- El share mode usa el jugador actualmente seleccionado en el picker (curUN).
    -- NO intenta auto-detectar al líder. El usuario lo selecciona manualmente.
    local btnShareMode  -- forward declaration
    local function updateShareBtn()
        if not btnShareMode then return end
        if shareModeActive then
            -- Mostrar a quién se le están usando los bloques
            local displayName = (curUN == LP.Name) and "tus bloques" or curUN
            btnShareMode.Text             = "SHARE ON: " .. displayName
            btnShareMode.BackgroundColor3 = T.ok or Color3.fromRGB(0, 180, 60)
            btnShareMode.TextColor3       = T.bg or Color3.new(0,0,0)
        else
            btnShareMode.Text             = "SHARE: OFF"
            btnShareMode.BackgroundColor3 = T.btnAlt
            btnShareMode.TextColor3       = T.text
        end
    end
    ...
    btnShareMode = btn(shareRow, "SHARE: OFF", UDim2.new(1,0,0,26), UDim2.new(0,0,0,0), T.btnAlt)
    btnShareMode.TextSize = 9

    btnShareMode.MouseButton1Click:Connect(function()
        if curUN == LP.Name then
            -- No tiene sentido activar share mode apuntando a uno mismo
            if SaveUI.prevStatus then
                SaveUI.prevStatus.Text = "Selecciona al líder en el picker primero"
        ...
        if SaveUI.prevStatus then
            if shareModeActive then
                SaveUI.prevStatus.Text = "Share ON: usando bloques de " .. curUN
                SaveUI.prevStatus.TextColor3 = T.ok or Color3.fromRGB(0,180,60)
            else
    ...
    SaveUI.nameInp = box(saveRow, UDim2.new(1,-80,0,26), UDim2.new(0,0,0,0), "")
    SaveUI.nameInp.PlaceholderText = "Nombre del build..."
    SaveUI.btnSaveNow = btn(saveRow, "Captura", UDim2.new(0,76,0,26), UDim2.new(1,-74,0,0), T.build)
    SaveUI.btnSaveNow.TextSize = 10

    lbl(secSave, "Mis Builds", UDim2.new(1,0,0,12), nil, T.sub).LayoutOrder = 4

    SaveUI.listScroll = mk("ScrollingFrame", secSave, {
        Size=UDim2.new(1,0,0,20), BackgroundTransparency=1, BorderSizePixel=0,
        ScrollBarThickness=4, ScrollBarImageColor3=T.purple,
        AutomaticCanvasSize=Enum.AutomaticSize.Y, CanvasSize=UDim2.new(0,0,0,0), LayoutOrder=5
    })
    pad(SaveUI.listScroll, 2,2,2,2)
    SaveUI.listCont = mk("Frame", SaveUI.listScroll, {
        Size=UDim2.new(1,0,0,1), AutomaticSize=Enum.AutomaticSize.Y, BackgroundTransparency=1
    })
    mk("UIListLayout", SaveUI.listCont, {Padding=UDim.new(0,4)})

    local secPrev = sec(PageSave, 4)

    local rSP = mk("Frame", secPrev, {Size=UDim2.new(1,0,0,28), BackgroundTransparency=1, LayoutOrder=2})
    SaveUI.BtnSavePrev = btn(rSP, "Vista previa: On", UDim2.new(1,-80,1,0), nil, T.accent)
    SaveUI.BtnSavePrev.TextColor3 = T.bg
    local spAlphaFrame = mk("Frame", rSP, {
        Size=UDim2.new(0,76,1,0), Position=UDim2.new(1,-76,0,0), BackgroundColor3=T.btnAlt, BorderSizePixel=0
    })
    corner(spAlphaFrame, 6)
    local spAlphaDown = btn(spAlphaFrame, "-", UDim2.new(0,22,1,0), UDim2.new(0,0,0,0), T.btnAlt)
    local spAlphaLbl  = lbl(spAlphaFrame, math.floor(savePrevAlpha*100).."%", UDim2.new(0,28,1,0), UDim2.new(0,22,0,0), T.text)
    spAlphaLbl.TextXAlignment=Enum.TextXAlignment.Center; spAlphaLbl.Font=Enum.Font.GothamBold; spAlphaLbl.TextSize=10
    ...
    SaveUI.BtnSaveMove = btn(sTRow, "", UDim2.new(0,38,0,24), UDim2.new(0,0,0,0), T.btn)
    mk("ImageLabel", SaveUI.BtnSaveMove, {Size=UDim2.new(0,18,0,18), Position=UDim2.new(0.5,-9,0.5,-9), BackgroundTransparency=1, Image=ICON_MOVE})
    SaveUI.saveMoveStep = box(sTRow, UDim2.new(0,44,0,24), UDim2.new(0,42,0,0), "1")
    SaveUI.saveMoveStep.TextXAlignment = Enum.TextXAlignment.Center
    SaveUI.BtnSaveRot = btn(sTRow, "", UDim2.new(0,38,0,24), UDim2.new(0,92,0,0), T.btnAlt)
    mk("ImageLabel", SaveUI.BtnSaveRot, {Size=UDim2.new(0,18,0,18), Position=UDim2.new(0.5,-9,0.5,-9), BackgroundTransparency=1, Image=ICON_ROT})
    SaveUI.saveRotStep = box(sTRow, UDim2.new(0,44,0,24), UDim2.new(0,134,0,0), "15")
    SaveUI.saveRotStep.TextXAlignment = Enum.TextXAlignment.Center
    SaveUI.btnScaleDown = btn(sTRow, "-", UDim2.new(0,22,0,24), UDim2.new(1,-72,0,0), T.btnAlt)
    SaveUI.scaleLbl     = lbl(sTRow, "1.0x", UDim2.new(0,26,0,24), UDim2.new(1,-48,0,0), T.text)
    SaveUI.scaleLbl.TextXAlignment = Enum.TextXAlignment.Center
    ...
    SaveUI.btnBuildSaved = btn(secPrev, "CONSTRUIR GUARDADO", UDim2.new(1,0,0,32), nil, T.purple)
    SaveUI.btnBuildSaved.LayoutOrder = 6
    SaveUI.prevStatus = mk("TextLabel", SaveUI.btnBuildSaved, {
        Size=UDim2.new(0,100,0,12), Position=UDim2.new(1,-104,1,-16),
        Text="", TextColor3=T.text, BackgroundTransparency=1,
    ...
    local function refPlaceBtns()
        SaveUI.BtnPlaceZone.BackgroundColor3 = useCZ and T.accent or T.btnAlt
        SaveUI.BtnPlaceZone.TextColor3       = useCZ and T.bg or T.text
        SaveUI.BtnSaveSel.BackgroundColor3   = (not useCZ) and T.accent or T.btnAlt
        SaveUI.BtnSaveSel.TextColor3         = (not useCZ) and T.bg or T.text
    end
    refPlaceBtns()
    ...
    SaveUI.btnScaleDown.MouseButton1Click:Connect(function()
        placeScale = math.max(0.1, placeScale-0.1)
        SaveUI.scaleLbl.Text = string.format("%.1fx", placeScale); renderSavePrev()
    end)
    SaveUI.btnScaleUp.MouseButton1Click:Connect(function()
        placeScale = math.min(10, placeScale+0.1)
        SaveUI.scaleLbl.Text = string.format("%.1fx", placeScale); renderSavePrev()
    end)
    ...
    SaveUI.BtnPlaceZone.MouseButton1Click:Connect(function()
        ...
        local z = closestZone(myRefPos())
        if z then placePosV = z.Position + Vector3.new(0,z.Size.Y/2+1,0)
        elseif myBaseCF then placePosV = myBaseCF.Position end
    ...
    renderSaveList = function(isAsync, onComplete)
        ...
        local function updateRow(i)
            ...
            local row = rowPool[i]
            if not row then
                row = mk("Frame", SaveUI.listCont, {
                    Size=UDim2.new(1,0,0,30), BackgroundColor3=T.input, LayoutOrder=i
                })
                corner(row, 6)
                local bar  = mk("Frame", row, {Name="Bar", Size=UDim2.new(0,4,1,-10), Position=UDim2.new(0,3,0,5), BackgroundColor3=T.accent, BorderSizePixel=0, Visible=false})
                corner(bar, 2)
                local nL   = lbl(row, "", UDim2.new(1,-96,1,0), UDim2.new(0,8,0,0), T.text); nL.Name="NameLbl"
                local useB = mk("TextButton", row, {Name="UseBtn", Size=UDim2.new(0,48,0,22), Position=UDim2.new(1,-92,0.5,-11), Text="Usar", TextColor3=T.text, BackgroundColor3=T.purple, Font=Enum.Font.GothamBold, TextSize=9})
                corner(useB, 4)
                local delB = mk("TextButton", row, {Name="DelBtn", Size=UDim2.new(0,40,0,22), Position=UDim2.new(1,-42,0.5,-11), Text="X", TextColor3=T.text, BackgroundColor3=T.danger, Font=Enum.Font.GothamBold, TextSize=11})
                corner(delB, 4)

                useB.MouseButton1Click:Connect(function()
                    local idx = row.LayoutOrder
                    selSaveIdx = idx; placeScale = 1; placeRot = CFrame.identity
                    SaveUI.scaleLbl.Text = "1.0x"
                    local z = closestZone(myRefPos())
                    if z then placePosV = z.Position + Vector3.new(0,z.Size.Y/2+1,0)
                    else placePosV = myRefPos() end
                ...
                delB.MouseButton1Click:Connect(function()
                    ...
                    local sv2 = Saves[idx]
                    if sv2 then
                        local fp = sv2._path or (FS2.."/"..sv2.name..".json")
                        FSys.del(fp); delSaveOrd(sv2.name)
                    end
                    table.remove(Saves, idx)
                    if selSaveIdx == idx then selSaveIdx = nil
                    elseif selSaveIdx and selSaveIdx > idx then selSaveIdx = selSaveIdx - 1 end
            ...
            if sv then
                ...
                local bar = row:FindFirstChild("Bar"); if bar then bar.Visible = isSel end
        ...
        if #Saves == 0 then
            ...
            if not emptyLbl then
                emptyLbl = mk("TextLabel", SaveUI.listCont, {
                    Name="EmptyLbl", Size=UDim2.new(1,0,0,18), LayoutOrder=1,
                    Text="(sin guardados)", TextColor3=T.sub, BackgroundTransparency=1,
                    Font=Enum.Font.Gotham, TextSize=10,
                    TextXAlignment=Enum.TextXAlignment.Center, TextYAlignment=Enum.TextYAlignment.Top
                })
            else emptyLbl.Text = "(sin guardados)" end
        ...
        local newH = math.clamp(#Saves*34, 34, 102)
        SaveUI.listScroll.Size = UDim2.new(1,0,0,newH)
        ...
        if isAsync then
            task.spawn(function()
                ...
                while i <= #Saves do
                    ...
                    i = batchEnd + 1; task.wait()
                end
                if myToken == renderToken then
                    for j = #Saves+1, #rowPool do if rowPool[j] then rowPool[j].Visible = false end end
                    if onComplete then onComplete() end
                end
            end)
        else
    ...
    SaveUI.btnSaveNow.MouseButton1Click:Connect(function()
        ...
        if not data then
            ...
            task.delay(1.5, function()
                if SaveUI.btnSaveNow and SaveUI.btnSaveNow.Parent then
                    SaveUI.btnSaveNow.Text="Captura"; SaveUI.btnSaveNow.BackgroundColor3=T.build
                end
            end)
            return
        end
        local nm = SaveUI.nameInp.Text
        nm = nm:gsub("%s+","_"):gsub("[^%w_%-]","")
        if nm=="" or nm=="_" then nm = autoName() end
        local used = {}
        for _,s in ipairs(Saves) do used[s.name] = true end
    ...
    SaveUI.btnBuildSaved.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            local ok2, err = pcall(function()
                ...
                -- ── Share mode: usa el jugador seleccionado manualmente ───────
                -- curUN = jugador elegido en el picker por el usuario
                local useShareMode = shareModeActive and (curUN ~= LP.Name)

                -- Determinar de quién son los bloques a usar
                local targetPlayerName = useShareMode and curUN or LP.Name

                -- Inventario a verificar (para saber si tiene el bloque disponible)
                local invFolder = useShareMode and getDataFolderFor(curUN) or dataFolder
                if not invFolder then
                    error(useShareMode
                        and ("No se pudo obtener inventario de " .. curUN .. ". ¿Sigue en el servidor?")
                        or "dataFolder no disponible")
                end

                -- userFolder donde aparecen los bloques recién colocados
                -- En share mode el servidor pone el bloque en el folder del jugador cuyos
                -- recursos se usaron (el seleccionado), no en el del LP.
                local targetFolder = userFolder(targetPlayerName)
                hookFolder2(targetFolder)
                ...
                local function pOB(pd)
                    ...
                    -- Verificar inventario del jugador correcto
                    local inv = invFolder:FindFirstChild(nm)
                    ...
                    -- Refrescar targetFolder si fue destruido
                    if not targetFolder or targetFolder.Parent == nil then
                        targetFolder = userFolder(targetPlayerName)
                        hookFolder2(targetFolder)
                    end

                    -- Colocar el bloque usando el RF del BuildingTool
                    ...
                    if blk then
                        ...
                        -- Escalar: el bloque ya existe con el CFrame correcto,
                        -- simplemente le pasamos el tamaño deseado.
                        -- En share mode el bloque fue creado con los recursos del líder
                        -- así que sRF2 puede escalarlo sin problema.
    ...
    Players.PlayerRemoving:Connect(function(p)
        task.defer(function()
            if whoVis then refreshWho() end
            -- Si el jugador seleccionado como "líder" en share mode se fue, desactivar
            if shareModeActive and p.Name == curUN then
                shareModeActive = false
                updateShareBtn()
                if SaveUI.prevStatus then
                    SaveUI.prevStatus.Text = p.Name .. " salió: share mode apagado"
                    SaveUI.prevStatus.TextColor3 = T.danger
                end
            end
        end)
            ...
            task.spawn(function()
                if SaveUI.listCont then
                    ...
                    if not emptyLbl then
                        emptyLbl = mk("TextLabel", SaveUI.listCont, {
                            Name="EmptyLbl", Size=UDim2.new(1,0,0,18), LayoutOrder=1,
                            Text="Cargando guardados...", TextColor3=T.sub, BackgroundTransparency=1,
                            Font=Enum.Font.Gotham, TextSize=10,
                            TextXAlignment=Enum.TextXAlignment.Center, TextYAlignment=Enum.TextYAlignment.Top
                        })
                    else emptyLbl.Text = "Cargando guardados..." end
                    emptyLbl.Visible = true
                end
                ...
                for i = 1, total do
                    ...
                    if name then
                        ...
                        if not ok or not res then ok, res = pcall(readBF, path) end

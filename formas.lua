...
function FormasModule.init(ENV)
    ...
    local gS,SHAPES
    do
        -- Fuentes de letras (Matrices 5x5)
        local FONT_UPPER = {
            ...
            [" "] = {"00000","00000","00000","00000","00000"}, ["!"] = {"00100","00100","00100","00000","00100"},
            ["?"] = {"01110","10001","00110","00000","00100"}, ["."] = {"00000","00000","00000","00000","00100"},
            [","] = {"00000","00000","00000","00100","01000"}, ["-"] = {"00000","00000","11111","00000","00000"},
            ["'"] = {"00100","00100","00000","00000","00000"},
        }
        local FONT_LOWER = {
            a = {"00000","00000","01110","10001","01111"}, b = {"10000","10000","10110","11001","10110"},
            c = {"00000","00000","01110","10000","01110"}, d = {"00001","00001","01101","10011","01101"},
            e = {"00000","00000","01110","11100","01110"}, f = {"00110","01000","11100","01000","01000"},
            g = {"00000","01101","10011","01111","00011"}, h = {"10000","10000","10110","11001","10001"},
            i = {"00100","00000","01100","00100","01110"}, j = {"00010","00000","00110","00010","10100"},
            k = {"10000","10000","10010","10100","11001"}, l = {"01100","00100","00100","00100","01110"},
            m = {"00000","00000","11011","10101","10001"}, n = {"00000","00000","10110","11001","10001"},
            o = {"00000","00000","01110","10001","01110"}, p = {"00000","11110","10001","11110","10000"},
            q = {"00000","01101","10011","01111","00001"}, r = {"00000","00000","10110","11000","10000"},
            s = {"00000","00000","01110","00010","11100"}, t = {"01000","01000","11100","01000","00110"},
            u = {"00000","00000","10001","10001","01101"}, v = {"00000","00000","10001","10001","01010"},
            w = {"00000","00000","10001","10101","01010"}, x = {"00000","00000","10001","01010","10001"},
            y = {"00000","10001","01010","00100","01000"}, z = {"00000","00000","11110","01000","11110"},
        }
        local function getCharMatrix(c)
            if FONT_LOWER[c] then return FONT_LOWER[c] end
            local up = c:upper()
            if FONT_UPPER[up] then return FONT_UPPER[up] end
            return FONT_UPPER[" "]
        end
        ...
        local function buildText(plan, cx, cy, cz, P)
            ...
            local words = {}
            for w in string.gmatch(txt, "%S+") do table.insert(words, w) end
            if #words == 0 then return end

            local rows = {}
            local wordsPerRow = P.rowMaxWords > 0 and P.rowMaxWords or #words
            for i, w in ipairs(words) do
                if i % wordsPerRow == 1 then table.insert(rows, {}) end
                table.insert(rows[#rows], w)
            end

            local charSize = P.charSize
            local stretchY = P.stretchY
            local spacing = P.spacing
            local th = P.thick

            local rowHeight = 5 * charSize * stretchY
            local rowGap = 2 * charSize * stretchY
            local totalHeight = #rows * rowHeight + (#rows - 1) * rowGap
            ...
            for r, row in ipairs(rows) do
                ...
                for i, w in ipairs(row) do
                    for c in w:gmatch(".") do width = width + 5 * charSize + spacing end
                    width = width - spacing
                    if i < #row then width = width + 3 * charSize + spacing end
                end
                rowWidths[r] = width
                if width > maxRowWidth then maxRowWidth = width end
            end

            local startX = cx - maxRowWidth / 2
            local startZ = cz - totalHeight / 2

            local boldMul = P.fontType == "bold" and 1.2 or 1.05
            local partSize = charSize * boldMul
            local partSizeY = charSize * stretchY * boldMul

            for r, row in ipairs(rows) do
                local currentX = startX + (maxRowWidth - rowWidths[r]) / 2
                local currentZ = startZ + (r - 1) * (rowHeight + rowGap)

                for i, w in ipairs(row) do
                    for c in w:gmatch(".") do
                        local matrix = getCharMatrix(c)
                        for py=1, 5 do
                            for px=1, 5 do
                                if matrix[py]:sub(px, px) == "1" then
                                    local pxX = currentX + (px - 0.5) * charSize
                                    local pxZ = currentZ + (py - 0.5) * charSize * stretchY
                                    plan[#plan+1] = {
                                        cframe = CFrame.new(pxX, cy, pxZ),
                                        size = Vector3.new(partSize, th, partSizeY)
                                    }
                                end
                            end
                        end
                        currentX = currentX + 5 * charSize + spacing
                    end
                    currentX = currentX + 3 * charSize + spacing
                end
            end
        end
    ...
    local rP=function()
        return {
            ...
            text = BInput.bTextStr and BInput.bTextStr.Text or "HOLA",
            charSize = math.max(0.5, tonumber(BInput.bCharSize and BInput.bCharSize.Text) or 2),
            stretchY = math.max(0.1, tonumber(BInput.bCharStretchY and BInput.bCharStretchY.Text) or 1),
            spacing = math.max(0, tonumber(BInput.bSpacing and BInput.bSpacing.Text) or 1),
            rowMaxWords = math.max(0, math.floor(tonumber(BInput.bRowMaxWords and BInput.bRowMaxWords.Text) or 0)),
            fontType = cFont,
        }
    end
    ...
    do
        local rRad=bRow(24, function() local d=SHAPES[sS]; return d and not d.isText end); BInput.bRadius=mkNumRow(rRad,"Radio","20",1,0.01)
        ...
        local visText = function() local d=SHAPES[sS]; return d and d.isText==true end
        local rTxt = bRow(24, visText)
        lbl(rTxt,"Texto",UDim2.new(0,50,1,0),nil,T.sub)
        BInput.bTextStr = box(rTxt, UDim2.new(1,-58,0,24), UDim2.new(0,52,0,0), "HOLA MUNDO")
        BInput.bTextStr.TextXAlignment = Enum.TextXAlignment.Left
        BInput.bTextStr.FocusLost:Connect(mPv)

        local rChar = bRow(24, visText)
        BInput.bCharSize = mkNumRow(rChar, "Tam. Letra", "2", 0.5, 0.5)

        local rStretch = bRow(24, visText)
        BInput.bCharStretchY = mkNumRow(rStretch, "Altura Y", "1", 0.1, 0.1)

        local rSpace = bRow(24, visText)
        BInput.bSpacing = mkNumRow(rSpace, "Espacio", "1", 0.1, 0)

        local rRowW = bRow(24, visText)
        BInput.bRowMaxWords = mkNumRow(rRowW, "Palabras", "0", 1, 0)

        local rFont = bRow(26, visText)
        lbl(rFont,"Fuente",UDim2.new(0,50,1,0),nil,T.sub)
        local fNormal = btn(rFont, "Normal", UDim2.new(0,70,0,24), UDim2.new(0,52,0,0), T.build)
        local fBold = btn(rFont, "Gruesa", UDim2.new(0,70,0,24), UDim2.new(0,126,0,0), T.btnAlt)
        local function rfFont()
            fNormal.BackgroundColor3 = (cFont=="normal") and T.build or T.btnAlt
            fBold.BackgroundColor3 = (cFont=="bold") and T.build or T.btnAlt
            mPv()
        end
        fNormal.MouseButton1Click:Connect(function() cFont="normal"; rfFont() end)
        fBold.MouseButton1Click:Connect(function() cFont="bold"; rfFont() end)
    end
